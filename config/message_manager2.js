 ...
 * @param {Array<Message>} edited_message           the message object edited by the calling user. null if no changes
 * @param {boolean} status_update                   wall status has changed (true | false).
 */
Mm.prototype.putUpdate = function(wall_id, question_id, nickname, edited_message, status_update) {
    ...
    */
    ...
    // Modified to take only a single message
    // Make note of my changes in other users' lists on the same question
    if (edited_message !== null) {
        var messages = this.data[wall_id].messages[question_id];
        for (var key in messages) {
            if (messages.hasOwnProperty(key) && key !== nickname) {
                messages[key][edited_message._id] = edited_message;
            }
        }
    }
    ...
    if (isTeacher) {
        ...
        for (var key in nicknames) {
            if ( nicknames.hasOwnProperty(key) && (timeNow - nicknames[key] > common.Constants.POLL_USER_EXPIRY_TIME)) {
                ...
                delete this.data[wall_id].messages[question_id][key];
            }
        }
    }
