    ...
    var DataService = (function () {
        function DataService($http, $window, $routeParams, $location, $interval, $mdDialog, $translate, utilityService, urlService, $mdMedia, constants) {
            ...
            this.data = {
                ...
                status: {
                    ...
                    nickname: null,
                    participants: [],
                    questionToEdit: new TalkwallApp.Question(''),
                    messageToEdit: null,
                    messageOrigin: null,
                    updateOrigin: false,
        ...
        DataService.prototype.checkAuthentication = function (successCallbackFn, errorCallbackFn) {
            ...
            this.$window.onbeforeunload = function () {
                ...
                _this.$http.get(url + 'disconnect/' + _this.data.status.nickname + '/' + _this.data.wall.pin + '/' + _this.data.question._id)
                    .then(function () {
        ...
        DataService.prototype.requestUser = function (successCallbackFn, errorCallbackFn) {
            ...
            this.$http.get(this.urlService.getHost() + '/user')
                ...
                _this.data.user = data[resultKey];
        ...
        DataService.prototype.requestWall = function (wallId, successCallbackFn, errorCallbackFn) {
            ...
            this.$http.get(this.urlService.getHost() + '/wall/' + wallId)
                .success(function (data) {
                var resultKey = 'result';
                _this.data.wall = data[resultKey];
        ...
        DataService.prototype.addQuestion = function (successCallbackFn, errorCallbackFn) {
            ...
            this.$http.post(this.urlService.getHost() + '/question', { wall_id: this.data.wall._id, question: this.data.status.questionToEdit })
                .success(function (data) {
                var resultKey = 'result';
                _this.data.wall.questions.push(data[resultKey]);
                // If this was the first question
                if (_this.data.wall.questions.length === 0) {
        ...
        DataService.prototype.deleteQuestion = function (question, successCallbackFn, errorCallbackFn) {
            ...
            this.$http.get(this.urlService.getHost() + '/messages/' + question._id)
                ...
                if (data[resultKey].length === 0) {
                    ...
                    var deleted_question_index = _this.utilityService.getQuestionIndexFromWallById(question._id, _this.data.wall);
        ...
        DataService.prototype.addMessage = function (successCallbackFn, errorCallbackFn) {
            ...
            this.$http.post(this.urlService.getHost() + '/message', {
                ...
                .success(function (data) {
                var resultKey = 'result';
                _this.data.question.messages.push(data[resultKey]);
                ...
                if (_this.data.status.updateOrigin) {
                    ...
                    _this.$http.put(_this.urlService.getHost() + '/message', {
                        message: _this.data.status.messageOrigin,
                        pin: _this.data.wall.pin,
                        ...
                        var idKey = '_id';
                        for (var i = 0; i < _this.data.question.messages.length; i++) {
                            if (_this.data.question.messages[i][idKey] === data[resultKey][idKey]) {
                                _this.data.question.messages.splice(i, 1);
                                _this.data.question.messages.splice(i, 0, data[resultKey]);
                            }
                        }
                    })
                        .catch(function (error) {
        ...
        DataService.prototype.getMessages = function () {
            ...
            if (this.data.question !== null) {
                this.$http.get(this.urlService.getHost() + '/messages/' + this.data.question._id)
                    .success(function (data) {
                    var resultKey = 'result';
                    _this.data.question.messages = data[resultKey];
                    _this.buildTagArray();
                })
                    .catch(function (error) {
                ...
                this.$http.put(this.urlService.getHost() + '/message', {
                    ...
                    for (var i = 0; i < _this.data.question.messages.length; i++) {
                        if (_this.data.question.messages[i][idKey] === data[resultKey][idKey]) {
                            ...
                            _this.data.question.messages[i][idKey].updateMe(data[resultKey]);
                            _this.parseMessageForTags(data[resultKey]);
                            break;
                        }
                    }
        ...
        DataService.prototype.processUpdatedMessages = function (pollUpdateObject) {
            ...
            if (this.data.status.participants.indexOf(this.data.status.nickname) === -1) {
                ...
                if (this.data.status.authorised) {
                    ...
                    for (var i = 0; i < this.data.status.participants.length; i++) {
                    ...
                    if (refreshNeeded && this.data.wall !== null && this.data.question !== null) {
                        this.$http.get(this.urlService.getHost() + '/wall/' +
                            ...
                            var resultKey = 'result';
                            _this.data.status.participants = data[resultKey];
                        })
                            .catch(function (error) {
            ...
            if (!this.data.status.authorised && pollUpdateObject.status.last_update > this.data.status.last_update) {
                ...
                this.getClientWall({ nickname: this.data.status.nickname, pin: this.data.wall.pin }, function () {
                    ...
                    if (new_question_id !== 'none') {
                        var new_question_index = _this.utilityService.getQuestionIndexFromWallById(new_question_id, _this.data.wall);
            ...
            pollUpdateObject.messages.forEach(function (updated_message) {
                var old_message = _this.utilityService.getMessageFromQuestionById(updated_message._id, _this.data.question);
                if (old_message !== null) {
                    // this.utilityService.removeNull(updated_message);
                    //angular.extend(old_message, updated_message);
                    var idKey = '_id';
                    for (var i = 0; i < _this.data.question.messages.length; i++) {
                        if (_this.data.question.messages[i][idKey] === updated_message._id) {
                            _this.data.question.messages.splice(i, 1);
                            _this.data.question.messages.splice(i, 0, updated_message);
                            break;
                        }
                    }
                }
                else {
                    _this.data.question.messages.push(updated_message);
                }
            ...
            if (this.dataService.getWall() === null) {
                ...
                this.messageFilterByContributorOnBoard = function (message) {
                    return (!message.deleted &&
                        ...
                        message.board[handle_1.selectedContributor] !== undefined &&
                        handle_1.unselected_users.indexOf(message.creator) === -1 &&
                ...
                this.messageFilterByAuthorAndTag = function (message) {
                    return (!message.deleted && handle_1.unselected_users.indexOf(message.creator) === -1 && handle_1.messageTagsNotPresent(message));
                };
            }
        };
