...
(function (TalkwallApp) {
    ...
    var Message = (function () {
        ...
        Message.prototype.updateSubsections = function (newMessage) {
            if (typeof newMessage['board'] !== 'undefined' && newMessage['board'] !== null) {
                for (var nickname in newMessage['board']) {
                    if (newMessage['board'].hasOwnProperty(nickname)) {
                        // Update an existing nickname
                        if (this.board.hasOwnProperty(nickname)) {
                            this.board[nickname].updateMe(newMessage['board'][nickname]['xpos'], newMessage['board'][nickname]['ypos'], newMessage['board'][nickname]['highlighted']);
                        }
                        else {
                            this.board[nickname] = new Nickname(newMessage['board'][nickname]['xpos'], newMessage['board'][nickname]['ypos'], newMessage['board'][nickname]['highlighted']);
                        }
                    }
                }
...
(function (TalkwallApp) {
    ...
    var DataService = (function () {
        ...
        DataService.prototype.setRefreshCallback = function (callback) {
        ...
        DataService.prototype.processUpdatedMessages = function (pollUpdateObject) {
            ...
            */
            ...
            pollUpdateObject.messages.forEach(function (updated_message) {
                var message = _this.utilityService.getMessageFromQuestionById(updated_message._id, _this.data.question);
                if (message !== null) {
                    // Message exists and needs to be updated
                    message.updateMe(updated_message);
                }
                else {
                    message = new TalkwallApp.Message().updateMe(updated_message);
                    _this.data.question.messages.push(message);
                }
                if (message.deleted) {
                    var counter = 0;
                    for (var i = 0; i < _this.data.question.messages.length; i++) {
                        if (_this.data.question.messages[i].creator === message.creator) {
                            counter++;
                        }
                    }
                    if (counter === 1) {
                        // This was the last message from its contributor, so remove from contributor list
                        _this.data.status.contributors.splice(_this.data.status.contributors.indexOf(message.creator), 1);
                    }
                }
                else {
                    if (_this.data.status.contributors.indexOf(updated_message.creator) === -1) {
                        _this.data.status.contributors.push(updated_message.creator);
                    }
...
(function (TalkwallApp) {
    ...
    var FeedMessageController = (function () {
        ...
        FeedMessageController.prototype.persistPosition = function (x, y) {
    ...
    function linker(isolatedScope, element, attributes, ctrl) {
        ...
        if (isolatedScope.onBoard === 'true') {
            ...
            isolatedScope.$watch(function () { return ctrl.message.board[isolatedScope.selectedParticipant]; }, function (newValue) { positionCSS(); }, true);
        }
...
(function (TalkwallApp) {
    ...
    var WallController = (function () {
        function WallController(dataService, $mdSidenav, $mdBottomSheet, $translate, $timeout, urlService, $window, utilityService) {
            var _this = this;
            this.dataService = dataService;
            ...
            this.dataService.setRefreshCallback(function () {
            ...
            if (this.dataService.getWall() === null) {
                ...
                this.messageFilterByContributorOnBoard = function (message) {
                    return (!message.deleted &&
                        ...
                        message.board[handle_1.selectedParticipant] !== undefined &&
                        handle_1.unselected_contributors.indexOf(message.creator) === -1 &&
                ...
                this.messageFilterByAuthorAndTag = function (message) {
                    return (!message.deleted && handle_1.unselected_contributors.indexOf(message.creator) === -1 && handle_1.messageTagsNotPresent(message));
                };
            }
        };
        ...
        WallController.prototype.contributorToggle = function (item) {
            var idx = this.unselected_contributors.indexOf(item);
            if (idx > -1) {
        ...
        ;
        WallController.prototype.contributorIsChecked = function () {
        ...
        WallController.$inject = ['DataService', '$mdSidenav', '$mdBottomSheet', '$translate', '$timeout', 'URLService', '$window', 'UtilityService'];
